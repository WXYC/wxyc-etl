"""Charter-form PyO3 wrappers must accept None.

`fn(None) -> ""` is a load-bearing ergonomic — Python consumers commonly call
the normalizer on a DB column that may be NULL. The first cut of the WX-2
charter (0.2.0) made `to_storage_form` / `to_match_form` / `to_ascii_form`
raise `TypeError` on None, forcing every caller to add `name or ""` guards.
0.2.1 unified the contract to `Optional[str] -> ""`; this test pins it.
