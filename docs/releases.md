...
5. Builds + smoke-tests `ghcr.io/wxyc/wxyc-postgres:pg17` and `:pg16` (single-arch local build). The smoke suite lives in [`infra/wxyc-postgres/smoke-test.sh`](../infra/wxyc-postgres/smoke-test.sh) (`smoke-test.sh <image> <pg_major>`) so the exact CI assertions are reproducible locally: baked-dictionary integrity (rules/version SHA-256 vs. `data/` + `ts_lexize('wxyc_unaccent', 'café')`), the runtime-identical unset baseline (postgres user, SSL, `listen_addresses`), the `WXYC_PG_EXTRA_ARGS` tuning hook (multi-flag, last-wins, `source = command line`), and process-model + restart integrity. Then `docker/build-push-action@v6` pushes both arches (`linux/amd64`, `linux/arm64`) with floating + pinned tags.
